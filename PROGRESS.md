...
**Date:** 2026-05-09
**Sprint:** Sprint 5C — Shiprocket integration (shipping, tracking, StockMovement audit)
**Status:** CODE COMPLETE — typecheck + lint clean (same two pre-existing nursery warnings on Sprint-1 components, no new ones); verification (`pnpm build`, Shiprocket sandbox order create + AWB assign + ₹1 test transaction with tracking webhook replay, dev-server smoke of the rate-quote endpoint, default-warehouse seed) tracked in [PENDING.md](./PENDING.md). Only 5D (CI + Sentry + analytics + MSG91 + order-cleanup cron + WebhookEvent audit) remains in Sprint 5.

### Done this session

- **Shiprocket REST client** — [lib/shiprocket.ts](./lib/shiprocket.ts) is a thin `fetch`-based wrapper, no SDK dependency, with in-memory token caching that refreshes on 401 (Shiprocket JWTs last ~10 days; we proactively refresh at 9). Exposes `getServiceability`, `createShiprocketOrder` (the full `orders/create/adhoc` payload), `assignAwb` (auto-picks cheapest courier when `courierId` is omitted), `requestShiprocketPickup`, and `trackByAwb`. Every result is a `ShiprocketResult<T>` discriminated union (`{ok:true, data}` vs `{ok:false, status, reason}`) so call sites treat creds-missing / network / HTTP-5xx as best-effort and degrade gracefully. `isShiprocketConfigured()` is the single check that gates the whole pipeline.
- **Shipping service** — [lib/services/shipping.ts](./lib/services/shipping.ts) `createShipmentForOrder(orderId)` orchestrates the create-order → assign-AWB → request-pickup chain in three external calls and writes a single local `Shipment` row with `carrier`, `awb`, `trackingUrl` (`https://shiprocket.co/tracking/<awb>`), `status: 'CREATED'`, `estimatedDate` (parsed from Shiprocket's ETD), `weightGrams` (from Shiprocket's `applied_weight` if returned, else summed variant weights), and the full Shiprocket response in `raw`. Idempotent — already-existing Shipment for the order short-circuits. Best-effort throughout: a Shiprocket outage logs but never breaks the order flow.
- **Tracking webhook handler** — `applyTrackingUpdate` in the same file maps Shiprocket's free-text `current_status` (`In Transit`, `Picked Up`, `Out for Delivery`, `Delivered`, `RTO Delivered`, `Exception`, ...) onto our `ShipmentStatus` enum and, when the carrier moves us forward (rank-based), forwards `Order.status` too (e.g. courier `Picked Up` → `OrderStatus.SHIPPED`, courier `Delivered` → `OrderStatus.DELIVERED`). All inside one `prisma.$transaction`. After commit, fires `sendOrderShippedEmail` / `sendOrderDeliveredEmail` only on the actual transition (replays of the same status are no-ops). Backwards moves (e.g. webhook reports `In Transit` after we already saw `Out for Delivery`) silently no-op.
- **Webhook route** — [/api/webhooks/shiprocket](./app/api/webhooks/shiprocket/route.ts) verifies the `x-api-key` header against `SHIPROCKET_WEBHOOK_TOKEN` in constant time (`node:crypto.timingSafeEqual`). Production with no token denies; dev with no token accepts and warns. Always returns 200 (Shiprocket retries on non-2xx; the docs explicitly require 200). Skips return-shipment events (`is_return === 1`) since the customer-side return UI is Phase 2.
- **`adminTransition` extended** — on `CONFIRMED → PROCESSING`, fires `createShipmentForOrder(orderId)` after the TX commits (alongside the existing email-send hooks for `→ SHIPPED` / `→ DELIVERED`). The Shiprocket call lives outside the TX — external HTTP shouldn't hold a Postgres transaction open. Best-effort: a Shiprocket failure logs but the admin's transition still succeeds.
- **`sendOrderShippedEmail` extended** — now reads the latest `Shipment` for the order and passes `carrier`, `awb`, and `estimatedDelivery` (formatted in `en-IN`) into the template. Falls back to the same null behaviour Sprint 5B shipped when no shipment exists (e.g. Shiprocket creds missing, manual fulfillment).
- **Customer tracking display** — [/account/orders/[orderNumber]](./app/(account)/account/orders/[orderNumber]/page.tsx) gains a Tracking section above the timeline when at least one Shipment exists. Shows courier, AWB (mono font), formatted estimated delivery, status label, and a `Track` button that opens the Shiprocket tracking URL. AWB-pending state ("AWB will appear once the courier assigns it") shows when the local Shipment row exists but the assign-AWB call hasn't returned yet (transient state during partial Shiprocket failures).
- **Rate quote endpoint** — [/api/shipping/rates](./app/api/shipping/rates/route.ts) proxies Shiprocket's serviceability endpoint with Zod-validated `pickupPincode`, `deliveryPincode`, `weightKg`, `cod`, `declaredValue` query params. Returns `{ available, recommendedCourierId, couriers: [{ courierId, name, ratePaise, etd, isSurface, codAvailable }] }`. With Shiprocket creds missing, returns `{ available: false, reason: 'shiprocket_unconfigured', couriers: [] }`. Phase-1 customer checkout still uses the flat-tier engine (the [/shipping](./app/(shop)/shipping/page.tsx) policy advertises those rates); the rates endpoint is for admin/diagnostic use and Phase-2 carrier-selection UX.
- **StockMovement audit (Sprint 4 deferred → 5C done)** — [lib/services/inventory.ts](./lib/services/inventory.ts) `recordStockMovements({tx, type, lines, refType, refId, reason, createdBy})` writes batched audit rows against the default warehouse (resolved by `code` from `DEFAULT_WAREHOUSE_CODE` env var, default `'DEFAULT'`, cached after first lookup). Wired into `placeOrderForCheckout` (type `SALE`, `refType: 'Order'`) and `cancelOrder` (type `RETURN`, `refType: 'Order'`). Skipped silently with a warning if the warehouse doesn't exist yet — the variant.stock decrement/increment is the source of truth and never blocks. Backorder-allowed variants don't generate audit rows on placement (consistent with no-decrement behaviour).
- **Default warehouse seed** — [prisma/seed.ts](./prisma/seed.ts) now upserts a `code: 'DEFAULT'` Mumbai HQ warehouse at the start of `main()` so a fresh `pnpm db:seed` produces a working StockMovement audit immediately. The address is intentionally `TODO: Warehouse address line 1` so production launch must override it via env-driven config or a manual `UPDATE warehouse SET line1 = ...`.
- **`.env.example` extension** — fleshed out the Shiprocket section with comments (API user vs dashboard login, where to get the values), added `SHIPROCKET_PICKUP_LOCATION` (defaults to `Primary` matching the Shiprocket dashboard's default), `SHIPROCKET_WEBHOOK_TOKEN` (security token configured on Shiprocket dashboard → Webhooks), and the new `DEFAULT_WAREHOUSE_CODE` for inventory audit.
...
| `pnpm lint` (`biome lint .`) | ✅ exit 0, zero new warnings (same 2 pre-existing nursery `noArrayIndexKey` warnings on Sprint-1 components remain) |
| `pnpm prisma validate` | ✅ schema valid (no schema changes — `Shipment`, `Warehouse`, `StockMovement` already in the Sprint-0 schema) |
| `pnpm prisma generate` | ✅ client regenerated (no-op since no schema changes) |
| `pnpm build` | ⏸️ deferred — same Neon block as prior sprints. Shipping routes pin `runtime = 'nodejs'` for `node:crypto.timingSafeEqual` and the Shiprocket SDK-less `fetch` calls. |

### Up next — to take Sprint 5C from code-complete to merged

1. **`pnpm db:seed`** against a wired Neon — confirms the Warehouse upsert lands cleanly. (Or `pnpm prisma migrate dev --name init` if rolling forward from a fresh DB.)
2. **Shiprocket sandbox creds** — fill `SHIPROCKET_EMAIL`, `SHIPROCKET_PASSWORD`, `SHIPROCKET_WEBHOOK_TOKEN`, `SHIPROCKET_PICKUP_LOCATION` in `.env.local` from Shiprocket's API-user setup. The pickup location must already exist on Shiprocket's dashboard (Settings → Pickup Addresses) under the same nickname.
3. **`pnpm dev` smoke — order pipeline**: place an order, sign in as `SUPER_ADMIN`, transition CONFIRMED → PROCESSING. Watch stdout for: a Shiprocket login (only on first call after restart), a `POST /v1/external/orders/create/adhoc`, a `POST /v1/external/courier/assign/awb`, a `POST /v1/external/courier/generate/pickup`, and the local Shipment row INSERT. Confirm `/account/orders/<num>` shows the new Tracking section with courier + AWB + tracking link.
4. **`pnpm dev` smoke — tracking webhook**: hand-craft a `POST /api/webhooks/shiprocket` with `x-api-key: <SHIPROCKET_WEBHOOK_TOKEN>` and a body matching the Shiprocket `current_status: "Out for Delivery"` shape (sample in [docs/SRS.md](./docs/SRS.md) and the Shiprocket apidocs). Confirm `Shipment.status → OUT_FOR_DELIVERY` and `Order.status → OUT_FOR_DELIVERY`. Replay the same webhook — confirm no duplicate OrderEvent and no duplicate email. Replay an "earlier" status (`In Transit`) — confirm no-op (rank-based forward-only).
5. **`pnpm dev` smoke — rate quote**: `curl 'http://localhost:3000/api/shipping/rates?pickupPincode=400001&deliveryPincode=110001&weightKg=1&cod=0'` returns the Shiprocket courier list. With creds missing, returns `{ available: false, reason: 'shiprocket_unconfigured' }`.
6. **₹1 Razorpay-then-Shiprocket end-to-end** in sandbox — verifies the entire Sprint 4 → 5B → 5C pipeline survives a real round-trip.
7. **`pnpm build`** — first production build for Sprint 5C. The 4 new TS modules and 2 new routes should compile cleanly.

### What's still ahead in Sprint 5

- **5D — Platform** (GitHub Actions CI, Sentry, Vercel Analytics + GA4 + PostHog + Microsoft Clarity, plus the deferred polish: order-cleanup cron for abandoned PENDING orders, `WebhookEvent` audit log keyed by Razorpay's `x-razorpay-event-id` and Shiprocket's `awb`+`current_timestamp`, MSG91 SMS once DLT clears, admin-side "manual pickup" / "regenerate AWB" button, multi-warehouse routing, per-product package dimensions on the variant).

### Decisions made this session

- **Shiprocket REST client = `fetch` + token cache, no SDK** (Sprint 4 Razorpay precedent). Phase 1 needs five calls (login / serviceability / create-order / assign-AWB / pickup / track — six counting the tracking endpoint). The official Shiprocket Postman collection covers our needs without the npm-package overhead. Re-evaluate if returns / NDR / refunds get wired in Phase 2.
- **Token cached in module memory with a 9-day TTL** (Shiprocket JWT validity is 10 days). On 401 we refresh once and retry. Multi-instance deployments may want to stash the token in Redis to avoid each instance minting its own; revisit if the login-rate becomes an issue. The cache is process-local — restarting Next clears it.
- **`createShipmentForOrder` fires on `CONFIRMED → PROCESSING`** (not on order placement, not on `→ SHIPPED`). Order-placement happens before payment confirms (PENDING for online orders), so creating Shiprocket orders that early would fill the dashboard with un-paid junk. `SHIPPED` is too late — we need an AWB *before* the courier picks up. PROCESSING ("warehouse is packing") is the natural moment: assign-AWB + request-pickup happen here so the courier knows what to pick up; the admin's `→ SHIPPED` then just signals the package is out the door.
- **`request_pickup` is fired automatically right after `assign_awb`**. Phase 1 is single-warehouse and single-courier-per-order; deferring pickup adds an admin step with no operational benefit. Phase 2 can add a "pause pickup" admin toggle for batch dispatch. Documented in PENDING.md.
- **Shiprocket order-create lives OUTSIDE the order-placement TX** — same principle as Sprint 4's Razorpay create-order. External HTTP shouldn't hold a Postgres transaction open. We accept the small risk that the local Order moves to PROCESSING but the Shipment never gets created (logged as warning, admin can manually retry); the alternative (TX held open during three external calls) trades a tiny edge case for chronic lock contention.
- **Tracking webhook is forward-only via rank tables** (`SHIPMENT_FORWARD_RANK`, `ORDER_FORWARD_RANK`). Carriers occasionally emit out-of-order events ("In Transit" arriving after "Out for Delivery"). Without forward-only enforcement these would flap statuses + fire duplicate emails. With it, the worst case is we miss a transient backwards event in the audit log — acceptable. The tracking timeline uses the OrderEvent rows + the carrier's `scans[]` if displayed, so the user-visible history isn't lossy.
- **Webhook auth = `x-api-key` shared secret**, not HMAC. Shiprocket's docs offer only this auth shape ("Security token should be an x-api-key"). Constant-time compare; production-with-no-token denies; dev-with-no-token accepts. Strictly weaker than Razorpay's HMAC-SHA256-over-raw-body; acceptable because the impact is "force Shipment status updates" (not "induce payments"), and an attacker would need to know the AWB anyway.
- **Status mapping is string-fuzzy, not status-id-driven**. Shiprocket sends both `current_status` (free-text) and `current_status_id` (integer). The integer is theoretically stable but undocumented across courier partners and changes between versions. Lower-cased substring matching (`includes('delivered')`, `includes('out for delivery')`) is more durable across courier oddities and across the ~50 status strings their docs sample-show.
- **`StockMovement` writes inside the `placeOrderForCheckout` / `cancelOrder` TX** (not after). Sprint 4 explicitly deferred this because there was no warehouse; Sprint 5C seeds one. The audit row sharing the TX with the variant.stock change keeps them consistent — if the order-place TX rolls back, the audit row rolls back with it. Acceptable extra cost: a `SELECT warehouse WHERE code='DEFAULT'` on the first request after restart, then cached.
- **Phase-1 single warehouse keyed by `code`, not by id** — operationally simpler (env var is `DEFAULT_WAREHOUSE_CODE`, not a cuid), and lets the admin add warehouses without code changes. Multi-warehouse routing (per-pincode origin selection) is Phase 3.
- **Default package dimensions = 30×30×10 cm at 500 g/item fallback**. Reasonable fit for typical electronics in our seed catalogue. Per-product package dimensions on `ProductVariant` are a Phase-2 catalog enhancement; the schema doesn't have a dimension JSON yet. Marked in PENDING.md so the value is grep-able.
- **Pickup location nickname is env-driven (`SHIPROCKET_PICKUP_LOCATION`, default `"Primary"`)** — must already exist in Shiprocket's dashboard. Validated implicitly: a wrong nickname causes the create-order call to 4xx, which our best-effort path logs and skips.
- **Customer tracking section is pure RSC** — no real-time polling for status updates. The Shiprocket webhook already pushes status changes into our DB, so a page reload picks up the latest. Phase-2 can add an SSE / polling refresher if customers complain about staleness.
- **`/api/shipping/rates` is admin/diagnostic only in Phase 1** — customer-facing checkout still uses the flat-tier engine because the [/shipping](./app/(shop)/shipping/page.tsx) policy page advertises those rates. Surfacing variable Shiprocket quotes at checkout would require updating the policy + price-display strategy, which is a Phase-2 commercial decision (do we pass real costs to customers, or absorb-and-flat-price?).
