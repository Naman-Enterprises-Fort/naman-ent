...
**Date:** 2026-05-09
**Sprint:** Sprint 1 polish — Admin catalog management + on-demand revalidation + header search-suggest + pincode-check fix
**Status:** CODE COMPLETE — typecheck + lint clean (same two pre-existing nursery warnings; no new ones); verification (`pnpm build`, dev-server smoke of the new admin pages + the search-suggest dropdown + the wired pincode-check) tracked in [PENDING.md](./PENDING.md). Sprint 5D (CI + Sentry + analytics + MSG91 + order cleanup cron + WebhookEvent audit) is intentionally deferred — the user is handling that on production. Sprint 1 polish for **product** CRUD + Sprint 2 polish (auth hardening) still ahead.

### Done this session

- **Brand CRUD — full create / edit / delete flow** — [lib/services/admin/brands.ts](./lib/services/admin/brands.ts) handles slug uniqueness, IN_USE rejection (can't delete a brand with products), and revalidates `catalog:brand` + Home. [/api/admin/brands](./app/api/admin/brands/route.ts) (POST) + [/api/admin/brands/[id]](./app/api/admin/brands/[id]/route.ts) (PATCH/DELETE) gate every mutation with `requireRole('CATALOG_MANAGER', 'SUPER_ADMIN')` and validate via the Sprint-0 `createBrandSchema` / `updateBrandSchema`. The admin UI at [/admin/brands](./app/(admin)/admin/brands/page.tsx) gains a "New brand" button and clickable rows linking to [/admin/brands/[id]](./app/(admin)/admin/brands/[id]/page.tsx); the new + edit pages share [components/admin/brand-form.tsx](./components/admin/brand-form.tsx) (react-hook-form + Zod resolver, name → slug auto-generation until the user manually edits the slug, optional Cloudinary URL for the logo until the upload widget lands in Phase 2).
- **Category CRUD — full create / edit / delete flow** — same shape as Brand plus a parent picker that surfaces every category as an `<option>`, with descendants of the current category marked `disabled` and a `forbidden` flag in the option label so it's obvious why those branches are off-limits (cycle prevention). [lib/services/admin/categories.ts](./lib/services/admin/categories.ts) walks the parent chain on update to defensively re-check; the seeded `assertNoCycle` short-circuits if it ever encounters an existing DB cycle so a corrupt state can't infinite-loop. Delete refuses if the category has products *or* children to avoid orphaning either side. Revalidates `catalog:category` + `/`, `/category`, `/category/[...slug]`. Admin UI at [/admin/categories](./app/(admin)/admin/categories/page.tsx) + [components/admin/category-form.tsx](./components/admin/category-form.tsx).
- **On-demand cache revalidation** — both admin services call `revalidateTag(...)` and the appropriate `revalidatePath(...)` after every mutation. Next.js 16's `revalidateTag` API now requires a cache profile argument; we pass `'max'` so the tagged entries refresh aggressively, matching the on-demand semantics customers expect from an admin save button. The Sprint-1 `unstable_cache` wrappers in [lib/services/catalog.ts](./lib/services/catalog.ts) (`getCategoryTree`, `getActiveBrands`, `getFeaturedCategories`, `getTrendingProducts`) already declare these tags so the wiring is end-to-end.
- **Header search-suggest dropdown** — [components/shop/search-bar.tsx](./components/shop/search-bar.tsx) now accepts a `suggest?` prop. When enabled, the input becomes a `role="combobox"` with a 150ms debounced fetch to `/api/search?mode=suggest&q=...&limit=6` (the route already existed from Sprint 1). The dropdown supports keyboard nav (`ArrowUp`/`ArrowDown` to highlight, `Enter` to navigate, `Escape` to close), click-outside dismissal, and an in-flight `AbortController` so racing keystrokes don't paint the wrong list. Mounted on the desktop header search bar at the top of [components/shop/header.tsx](./components/shop/header.tsx). Mobile (the search row below the header) keeps the simple form-submit behaviour to avoid a screen-eating dropdown on small viewports.
- **Pincode-check wired to `/api/serviceability`** — [components/shop/pincode-check.tsx](./components/shop/pincode-check.tsx) on the PDP no longer calls India Post directly with a hardcoded "3–5 business days" ETA. The Sprint 4 stale TODO is now resolved: it now hits the internal `/api/serviceability` endpoint, which returns the same India-Post-derived city/state plus the metro same-day eligibility flag. The success copy switches between "delivers to … in 3–5 business days" (standard), "in 2–3 days (express)" (when express is available), and "eligible for same-day delivery" (metro pincodes — Delhi / Mumbai / Bengaluru / Kolkata / Chennai / Hyderabad). Non-serviceable pincodes show an explicit error.
...
1. **`pnpm dev` smoke — admin Brand**: visit `/admin/brands`, click "New brand", enter name (slug auto-fills), optional logo URL, save. Confirm the new brand appears in the list. Click into the row, edit the name, save. Try to delete a brand that has products → expect 409 with the IN_USE message. Try to create a brand with an existing slug → expect 409 SLUG_TAKEN.
2. **`pnpm dev` smoke — admin Category**: same flow at `/admin/categories`. Pick a parent. Try to set a parent that's a descendant (the option is disabled in the dropdown, but verify the server-side `assertNoCycle` rejects with 400 if you bypass the disabled state via devtools).
3. **`pnpm dev` smoke — search-suggest**: type "ip" in the desktop header search bar — expect the dropdown to render up to 6 suggestions matching iPhone seed products. ArrowDown / ArrowUp highlights, Enter navigates to the highlighted PDP, Escape closes. Plain text + Enter (no highlight) navigates to `/search?q=...`.
4. **`pnpm dev` smoke — pincode-check**: on a PDP, enter `110001` → expect "Delhi · eligible for same-day delivery" (metro). Enter `682001` → expect "Ernakulam · in 4–7 business days" (non-metro). Confirm same-day variants read "eligible for **same-day delivery**" rather than "in same-day delivery".
5. **On-demand revalidation smoke**: edit a brand's `isActive` flag and save → confirm the Home page brand strip updates on next reload (vs the 10-min ISR window). Same for categories: hide one and confirm `/category` reflects the change immediately.
6. **`pnpm build`** — first production build for this slice.

### What's still ahead

- **Sprint 1 polish — Product CRUD form** (separate PR): the largest remaining gap in admin tooling. The Zod schemas (`createProductSchema` / `updateProductSchema`) already exist with full variant + image + spec sub-schemas. The form needs: core fields, multi-select category picker, brand dropdown, variant editor (at least one default variant required), image URL list with `isPrimary` flag, spec rows. Cloudinary upload widget can defer to a separate Phase-2 polish.
- **Sprint 2 polish — Auth hardening** (separate PR): per-account login lockout after 5 failed attempts (currently only IP-rate-limited), Cloudflare Turnstile on `/register` and `/forgot-password` (env vars are already reserved at `NEXT_PUBLIC_TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY`).
...
- **Admin auth gating uses `requireRole('CATALOG_MANAGER', 'SUPER_ADMIN')`** consistently across mutation routes and on the new/edit pages. The pages additionally redirect to `/admin/dashboard` rather than 403 — a CATALOG_MANAGER who somehow lands on a non-existent admin path gets a softer experience. The proxy.ts gate already redirects unauthenticated users to `/login`.
- **Cycle prevention for categories is two-layered**: the edit page pre-computes descendant IDs (BFS over the `parentId` map) and disables them in the parent picker so the admin can't *try* to set a forbidden parent. The service layer also walks the parent chain on update via `assertNoCycle` — defensive, since the disabled `<option>` can be bypassed via devtools. The walk has a guard set so a corrupt DB cycle short-circuits rather than infinite-loops.
- **Slug auto-generation** until the user manually edits the slug field. The form watches the `name` field and rewrites `slug` via `slugify()` only while `slugTouched` is false. Once the admin types in the slug field, the rewrite stops. Reduces friction on the create flow without surprising admins who deliberately set a custom slug.
- **`revalidateTag(tag, 'max')` on every admin mutation**. Next.js 16's API now requires the cache profile arg; `'max'` matches the on-demand semantics of an admin save (refresh aggressively). Combined with `revalidatePath(...)` for the routes that aren't tag-wrapped (Home page, /category, etc.).
- **No optimistic UI on the admin forms** — submit → server roundtrip → `router.refresh()` + redirect. The list re-fetches on next render. Phase-2 polish could add toast notifications + optimistic updates, but Phase 1 prioritises correctness over snappiness for low-frequency admin actions.
- **Logo / image fields are URL-only inputs in Phase 1** (the schema already expected `z.url()`). The Cloudinary upload widget is operational follow-up tied to provisioning Cloudinary creds. Until that lands, admins paste a URL — same shape as the seeded placeholder images.
- **Search-suggest debounced at 150ms** with an in-flight `AbortController`. Faster than 100ms feels twitchy with India-Post-typical latency on the trgm query; slower than 200ms feels sluggish. 150ms is the sweet spot widely used in shipped autocompletes (Algolia, Stripe Atlas, etc.).
- **Suggest dropdown uses `<div role="listbox">` + `<div role="option" tabIndex={-1}>`** instead of `<ul>`/`<li>`. Biome's a11y rule `noNoninteractiveElementToInteractiveRole` flags interactive ARIA roles on list elements; the WAI-ARIA combobox/listbox spec actually expects div containers, with the input keeping focus and `aria-activedescendant` pointing at the highlighted option. A button child handles the click for non-keyboard users.
- **Mobile search-bar keeps the simple form-submit behaviour** — no dropdown on small viewports because it would consume too much screen real estate. Mobile users hit Enter and land on `/search?q=...` directly. Phase-2 can add a full-screen mobile-optimised suggest UI.
- **Pincode-check now hits `/api/serviceability`** (Sprint 4 endpoint), replacing the Sprint-1-era India-Post-direct fallback. The response includes the Sprint 4 metro flag, so the success copy gracefully advertises same-day eligibility for Delhi / Mumbai / Bengaluru / Kolkata / Chennai / Hyderabad pincodes — a real conversion-driver for those geographies.
...
- [ ] Product admin CRUD (title, slug, brand, description, MRP, selling price, GST %, HSN, country of origin, BEE rating, hazmat flags, status, images, specs, variants) — read-only table done, full create/edit form is the next Sprint-1-polish slice (Zod schemas already complete in `createProductSchema` / `updateProductSchema`).
