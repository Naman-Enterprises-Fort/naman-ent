...
**Date:** 2026-05-09
**Sprint:** Sprint 1 polish — Admin product CRUD (with variants, images, specs)
**Status:** CODE COMPLETE — typecheck + lint clean (same two pre-existing nursery warnings; no new ones); verification (`pnpm build`, dev-server smoke of the new product create/edit pages + variant diff/upsert/delete + image and spec replacement) tracked in [PENDING.md](./PENDING.md). Sprint 5D is intentionally deferred. Sprint 2 polish (auth hardening) is the next slice.

### Done this session

- **Product CRUD service** — [lib/services/admin/products.ts](./lib/services/admin/products.ts) handles the full lifecycle in a single `prisma.$transaction`. Create-flow: slug-uniqueness check, brand + category existence checks, SKU collision check across all submitted variants, then the nested `product.create` writes the product + ProductCategory rows + variants + images + specs together. Update-flow: replace-or-upsert. Categories are wholesale-replaced. Variants are diffed by `id` — existing IDs get updated; new entries (no `id`) are inserted; existing variants not in the submitted list are deleted, but a P2003 / P2014 FK violation (carts / orders referencing the variant) surfaces as a typed `ProductError('VARIANT_IN_USE', ...)` with HTTP 409. Images and specs are wholesale-replaced (no FK refs). Soft delete sets `deletedAt` + `status: 'ARCHIVED'`; the catalog reads filter `deletedAt: null` and `OrderItem.productSnapshot` keeps order-history pages working post-archive.
- **Admin API routes** — [/api/admin/products](./app/api/admin/products/route.ts) (POST) + [/api/admin/products/[id]](./app/api/admin/products/%5Bid%5D/route.ts) (PATCH, DELETE) gated by `requireRole('CATALOG_MANAGER', 'SUPER_ADMIN')` and validated with the Sprint-0 `createProductSchema` / `updateProductSchema`. The schemas already enforce: at least one variant, at least one category (max 8), at most one default variant, at most one primary image, selling price ≤ MRP per variant.
- **Product form** — [components/admin/product-form.tsx](./components/admin/product-form.tsx) is the largest form in the app, built with `useFieldArray` for variants / images / specs (each row independently add/edit/remove). Sections: Basics (name, slug, brand, status, short + long description), Categories (multi-select checkbox grid), Tax/origin/warranty (HSN, country, BEE, warranty months/type/doc, model + MPN + GTIN, hazmat + box-contents as comma-separated), Variants (full editor — SKU, name, position, MRP, price, GST, stock, weight + dimensions, attributes via a `key=value`-per-line textarea, isDefault + backorder + lowStock), Images (URL + alt + position + isPrimary), Specs (group + key + value + position), SEO. Submit-time conversion: form-shape → schema-shape (CSV → array, attribute textarea → Record), then `createProductSchema.safeParse` for client-side guard before the network call. On 409 SLUG_TAKEN / SKU_TAKEN / VARIANT_IN_USE, the server's typed error message surfaces in the form's serverError banner.
- **Admin pages** — [/admin/products/new](./app/(admin)/admin/products/new/page.tsx) (auth-gated, fetches brand + category lists) + [/admin/products/[id]](./app/(admin)/admin/products/%5Bid%5D/page.tsx) (loads the full product with variants/images/specs and converts Decimal money to plain numbers for the form). The listing page at [/admin/products](./app/(admin)/admin/products/page.tsx) gains a "New product" button and clickable name → edit. Soft-archived products (`deletedAt != null`) drop out of the list automatically.
...
1. **`pnpm dev` smoke — create**: visit `/admin/products/new`, fill name (slug auto-fills), brand, status=ACTIVE, description, pick at least one category, fill the default variant (SKU + MRP + price + stock), add an image URL, save. Confirm the new product appears in the list and on `/products/<slug>`.
2. **`pnpm dev` smoke — edit**: open a seeded product. Adjust the price + stock on the default variant, save. Confirm the change reflects on the customer PDP within ~1s (on-demand revalidation, not the 1h ISR window).
3. **`pnpm dev` smoke — variants diff**: on an existing product, click "Add variant" → fill SKU/MRP/price/stock, save. Confirm the new variant lands. Click "Remove" on a variant that has carts/orders → expect 409 VARIANT_IN_USE; the form's serverError banner shows the carrier message.
4. **`pnpm dev` smoke — invariants**: try saving with two variants both marked "default" → 400 with the schema's "Only one variant can be marked default". Try a variant where price > MRP → 400 "Variant <SKU>: selling price cannot exceed MRP". Try saving with zero categories → 400 "Pick at least one category".
5. **`pnpm dev` smoke — soft delete**: click "Archive product" on the edit page. Confirm the product disappears from `/admin/products` (deletedAt set) and from public surfaces (Home / PLP / PDP), but `/account/orders/<num>` for an order containing this product still renders correctly (productSnapshot keeps the order page intact).
6. **`pnpm build`** — first production build with the new admin pages compiled.
...
- **Phase-2 polish** beyond launch: Cloudinary upload widget + per-product package dimensions (both unblock 5C improvements); admin "delete variant" with archive-instead-of-delete; bulk import via CSV; rich-text description editor (Tiptap per SRS §6.12.2).

### Decisions made this session

- **Variants diff by `id` on update**, not by `sku`. SKU changes are common during catalog cleanup; using id as the stable key lets admins rename SKUs without losing the row's history. New variants (no `id` on submit) get inserted; existing variants not in the submitted list get deleted, with a typed `VARIANT_IN_USE` rejection if FK refs prevent it.
- **Soft delete (sets `deletedAt` + `status: 'ARCHIVED'`)** rather than hard delete. The catalog reads filter `deletedAt: null`, so archived products vanish from public surfaces; `OrderItem.productSnapshot` (Sprint 4) keeps order-history pages working post-archive. Hard delete is impossible anyway because order items reference variants under `RESTRICT`.
- **Categories / images / specs are wholesale-replaced on update**; only variants are upserted-by-id. Categories are a thin M2M (no FK refs), images and specs have only `productId` FK so a delete-and-recreate is safe and simple. Wholesale replace also makes "drag to reorder" implementable in Phase 2 without changing the API contract.
- **Variant attributes round-trip via a `key=value`-per-line textarea** in the form, parsed to `Record<string, string>` before submit. A structured key-value-row editor would be nicer UX but pushes complexity into the form for marginal admin benefit; Phase 2 can graduate when SKUs commonly have >5 attributes.
- **Hazmat flags + box contents are comma-separated text inputs** for the same reason — array editors with add/remove rows would be heavyweight for fields that typically hold 0–3 items.
- **Variant `attributes` JSON field uses `Prisma.InputJsonValue` cast** rather than a typed Json wrapper. The schema enforces `Record<string, string>` at the validation layer, and Prisma's JSON typing is generic enough to accept the parsed object directly.
- **The form does its own `createProductSchema.safeParse` before the network call**, even though the server validates again. Catches type mismatches client-side without paying the network round-trip; the server-side validation is the trust boundary.
- **No client-side `zodResolver` on the form** — react-hook-form's resolver runs on the form-shape (with `attributesText` strings), not the schema-shape (with `attributes` records). Doing the conversion + parse inside `onSubmit` is simpler than a transform-based schema, and the manual approach surfaces a single first-issue message in the form's error banner rather than per-field hints. Phase-2 polish could move to per-field error display.
- **Soft-archive uses `status: 'ARCHIVED'` *plus* `deletedAt`** redundantly. The catalog reads filter on `deletedAt: null` so the archive flag is the trust boundary; the status enum is a UI hint for the admin table (so it badges "Archived" rather than "Active") and matches the SRS-defined Product status enum.
